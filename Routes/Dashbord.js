        ...
        Dashbord.findOne({
            ...
            if (dashbord)
                ...
                dashbord.save((err, data) => {
                    if (err) return returnMessage.globalOne(false, 400, "Image saving error", res, err);
                    else
...
router.get('/Dashbord', async (req, res) => {
    ...
    try {
        Dashbord.find((err, data) => {
            ...
            else return returnMessage.globalOne(false, 400, "No Images found", res, err);
        });
    } catch (ex) {
        return returnMessage.globalOne(false, 501, "server side error occurred! Please try again shortly..", res, ex);
    }
...
router.put('/Dashbord/:id', (req, res) => {
    ...
    try {
        ...
        Dashbord.findByIdAndUpdate(req.params.id, { $set: { imageUrl: req.body.imageUrl } }, { new: true, useFindAndModify: false }, (err, data) => {
            ...
            if (err) return returnMessage.globalOne(false, 400, "Image updating error", res, err);
